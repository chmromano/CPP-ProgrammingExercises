...
public:
    ...
    void advertise(const std::shared_ptr<Car> &car) { listing.push_back(car); }
...
public:
    ...
    void sell() {
        ...
        auto ci = std::find_if(cars.begin(), cars.end(),
                               [&license](const std::shared_ptr<Car> &c) { return license == c->GetLicense(); });
        if (ci != cars.end()) {
            cars.erase(ci);
        }
    }
    ...
    void add_site(const std::shared_ptr<Website> &w) { sites.push_back(w); }
};
