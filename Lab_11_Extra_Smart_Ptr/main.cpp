...
public:
    ...
    virtual void SetObserver(shared_ptr<CounterObserver> obs_) = 0;
};
...
private:
    int value;
    int upperLim;
    vector<shared_ptr<CounterObserver>> obs;

    void Notify() {
        for (const auto &observer: obs) {
...
public:
    ...
    void SetObserver(shared_ptr<CounterObserver> obs_) override {
        obs.push_back(shared_ptr<CounterObserver>(obs_));
    }
};
...
int main() {
    // Creating objects.
    shared_ptr<DerivedObserver> overflowObserver, limitedObserver, sharedObserver;
    overflowObserver = make_shared<DerivedObserver>("OVF-OBS");
    limitedObserver = make_shared<DerivedObserver>("LIM-OBS");
    sharedObserver = make_shared<DerivedObserver>("SHR-OBS");
